...
contains
  ...
  write(*,*) 'TJ checks finit=',finit, restfname
  !create simulations
  ...
  !testing only                                 -zlyu    
  !write(stdout, *) '***************************'
  !write(stdout, *) 'sbetrdrivermod 1) after forcing_data%ReadData', forcing_data%t_soi(1,1), forcing_data%t_soi(forcing_data%num_time,1), forcing_data%num_time !this%t_soi(time, level)
  !write(stdout, *) '***************************'
  ! end of the testing
  ...
  if(continue_run)then
    ...
    print*,'continue from restart file:',restfname
    call read_restinfo(restfname, nstep) !Why cannot read the restfname correctly?
  ...
  !testing only                                 -zlyu    
  !write(stdout, *) '***************************'
  !write(stdout, *) 'sbetrdrivermod 2) after forcing_data%UpdateForcing', lbj, ubj, forcing_data%t_soi(1,:)        !change here 
  !write(stdout, *) '***************************'
  ! end of the testing
  
  !write(stdout, *),'af init update',forcing_data%t_soi(1,:)
  !write(stdout, *),'initial water state variable output',time_vars%tstep
  ...
  if(trim(restfname)/='')then
    print*,'Before BeTRRestartOpen:',restfname
  ...
  do
   ...
   ! write(stdout, *) 'In sbetrDriverMod after BeTRSetBiophysForcing, first'                  !-zlyu
    ...
    ! Update forcing for each time step, so temperature_vars%t_soisno_col(c,j), c is grid, j is level
    call forcing_data%UpdateForcing(grid_data,  bounds, lbj, ubj, simulation%num_soilc, &
      simulation%filter_soilc, time_vars, col, pft, atm2lnd_vars, soilhydrology_vars, &
      soilstate_vars,waterstate_vars, waterflux_vars, temperature_vars, chemstate_vars, &
      plantMicKinetics_vars, simulation%jtops)
    ...
    !write(stdout, *) 'sbetrDriverMod inside loop checking step:', record, temperature_vars%t_soisno_col(1,1), temperature_vars%t_soisno_col(1,10)         
